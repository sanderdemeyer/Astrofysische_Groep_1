...
for integ in Integrators:
    ...
    for h in H:
        t, E =  np.loadtxt('energy/{}/{}_{}_100.000000_{:.6f}.txt'.format(system, system, integ, h), unpack=True)
        E_rel_error = abs((E - E[0])/E[0])
        plt.plot(t[:int(len(t)/2)], E_rel_error[:int(len(t)/2)], label=str(h))
    plt.yscale('log')
    plt.xlim(0, 50)
    plt.xlabel('t')
    plt.ylabel('$\Delta E_{rel}$')
